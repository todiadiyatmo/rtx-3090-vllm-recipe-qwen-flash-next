...
def _get_ple_embedding_quant_method(
    ...
    packed = _get_ple_embedding_quant_method_packed(
...
def _get_ple_embedding_quant_method_packed(
    quant_config: QuantizationConfig | None,
    prefix: str,
    ple_embedding_dtype: object = None,
) -> QuantizeMethodBase | None:
    """Cabang tambahan gpu-1: NVFP4 terkemas (vllm#53899) + gerbang INC (Task 74)."""
    ...
    # PATCH Task 67/74 (gpu-1): checkpoint auto-round/inc (target Intel W4A16) dengan
    # tabel PLE FP8 RadixArk (`ple_embedding_dtype: float8_e4m3fn`,
    # `ngram_embedding.weight_scale` global). Tanpa ini quant_method None -> param BF16 ->
    # tensor F8 di-cast tanpa skala = jawaban salah diam-diam (Task 39).
    if _ple_dtype_is_fp8(ple_embedding_dtype):
        logger.info_once(
            "PLE embedding %s: config declares FP8 table with quant_config %s, "
            "using the runtime FP8 method (gpu-1 patch)",
            prefix,
...
class Qwen4ExpNGramEmbedding(PleOffloadLayer):
    ...
    def forward_impl(  # type: ignore[override]
        ...
        del hidden_states
        if is_offload_process() or output_buffer is not None:
            # Proses offload tidak pernah di-capture cudagraph dan tidak punya
            # forward context untuk custom op; hitung langsung (vllm#53899).
